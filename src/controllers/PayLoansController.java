...
/**
 * Class providing the <b>controller</b> for the processes related to <b>paying loans</b>
 */
public class PayLoansController implements ActionListener, DocumentListener, WindowListener {
    /**
     * Graphical user interface related to paying loans
     */
    private PayLoans loansScr;
    /**
    ...
    /**
     * Creates a controller object with the graphical user interface related to paying
     ...
     */
    public PayLoansController(PayLoans loansScr, MainScreen mainScr, GameMaster game) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("Pay")) {
            int numLoans;               // number of loans that the player wants to pay
            ...
            try {
                ...
                errScr = new ErrorDialogLoan("Pay Loan", "Must be a whole number");
            }
            ...
            if (isNumber && numLoans < 0) {
                ...
                errScr = new ErrorDialogLoan("Pay Loan", "Must not be less than 0");
                loansScr.setAlwaysOnTop(true);

            } else if (isNumber && numLoans > game.getCurrNumLoans(game.getIDOf(game.getIndexCurrPlayer()))) {
                /* Display an error message if the number of loan that the player seeks to pay
                * is greater than his/her current number of loans */
                loansScr.clearTfInput();
                ErrorDialogLoan errScr;

                loansScr.setAlwaysOnTop(false);
                errScr = new ErrorDialogLoan("Pay Loan", "Must not be greater than number of loans");
                loansScr.setAlwaysOnTop(true);

            } else if (isNumber && numLoans <= game.getCurrNumLoans(game.getIDOf(game.getIndexCurrPlayer()))) {
                /* Allow the player to pay the specified number of loans. */
                game.payLoan(game.getIDOf(game.getIndexCurrPlayer()), numLoans);
                loansScr.setVisible(false);
                ...
                "New Cash Amount", "/assets/salaryIcon2.png");
